...
fn render(scale: u32, display_width: u32, display_height: u32, canvas: &mut Canvas<Window>, buffer: &Vec<u8>) {
    // Fill with buffer
    ...
    'running: loop {
        for event in sdl_context.event_pump().unwrap().poll_iter() {
            match event {
                ...
                Event::KeyDown {keycode: Some(i), ..}  => {
                    match i {
                        Keycode::X => {chip.keys[0] = true;}
        ...
        if current_time - last_cycle_time > 1 {
            ...
            render(video_scale, texture_width, texture_height, &mut canvas, &chip.video);
        }
    }
}
